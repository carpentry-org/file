...
Once a write on the file has failed, the error sticks: every later `flush` and
the eventual `close-checked` report it too, naming that earlier failure rather
than the call that surfaced it.
